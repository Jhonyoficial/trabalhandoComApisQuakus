...
public interface MyRemoteService {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    String consultarDadosConta(@HeaderParam("Authorization") String accessToken, ConsultaBoletoDTO consultaBoletoDTO);


    @POST
    @Path("/v5/transactions/billpayments")
    @Produces(MediaType.APPLICATION_JSON)
    String EfetuarPagamento(@HeaderParam("Authorization") String AccessToken, ConsultaBoletoDTO consultaBoletoDTO);
}
